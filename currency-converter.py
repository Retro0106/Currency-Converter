...
class CurrencyConverter:
    ...
    def convert(self, amount, base_currency, des_currency):
        if base_currency != 'EUR':
            amount /= self.rates.get(base_currency, 1)

        amount = round(amount * self.rates.get(des_currency, 1), 2)
        return '{:,}'.format(amount)  # Add commas for readability
...
class Main(tk.Tk):
    ...
    def __init__(self, converter):
        ...
        self.version_label.place(x=370, y=380, anchor='se')
        ...
        self.currency_variable1 = StringVar(value='USD')
        self.currency_variable2 = StringVar(value='IDR')
    ...
    def processed(self):
        try:
            ...
            formatted_amount = '{:,}'.format(given_amount)

            self.final_result.config(text=f'{formatted_amount} {given_base_currency} = {converted_amount} {given_des_currency}')
        except ValueError as e:
            messagebox.showwarning('WARNING!', str(e))

if __name__ == '__main__':
    converter = CurrencyConverter('https://api.exchangerate-api.com/v4/latest/EUR')
    app = Main(converter)
    app.mainloop()
